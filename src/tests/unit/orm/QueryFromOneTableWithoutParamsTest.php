...
class QueryFromOneTableWithoutParamsTest extends \Codeception\Test\Unit
{
    ...
    public function testQueryOrderAndLimit()
    {
        ...
        // limit + offset + order
        $expected = 'SELECT t.* FROM student t LIMIT 10 OFFSET 100 ORDER BY t.name ASC';
        $actual = $this->createQuery('student', 't', null, null, 10, 100, ['name', 'asc']);
        $this->assertSame($expected, $actual);

        // limit + order
        $expected = 'SELECT t.* FROM student t LIMIT 10 ORDER BY t.name ASC';
        $actual = $this->createQuery('student', 't', null, null, 10, null, ['name', 'asc']);
        $this->assertSame($expected, $actual);

        // order + first
        $expected = 'SELECT t.* FROM student t LIMIT 1 ORDER BY t.name ASC';
